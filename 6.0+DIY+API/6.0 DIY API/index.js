...
app.get("/random", (req,res) =>{
  const data = Math.floor(Math.random()*jokes.length);
  res.json(jokes[data]);
})
//2. GET a specific joke
app.get("/jokes/:id", (req,res) =>{
  const id = parseInt(req.params.id);
  const index = jokes.find((jokes) => jokes.id === id);
  res.json(index);
})
//3. GET a jokes by filtering on the joke type
app.get("/filter", (req,res) =>{
  const type = req.query.type;
  var index = jokes.filter((jokes) => jokes.jokeType === type);
  res.json(index);
})
//4. POST a new joke
...
app.put('/jokes/:id',(req,res)=>{
  ...
  jokes[jokes.findIndex((jokes) => jokes.id === id)] = newjoke;
  res.json(newjoke);
  res.send("Done")
})
//6. PATCH a joke
...
app.delete('/jokes/:id',(req,res)=>{
  ...
  const index = jokes.findIndex((jokes)=> jokes.id === id);
  if(index>-1)
    {jokes.splice(index,index);
  res.send(`${id} has been deleted successfully`);}
  else{
    res
    .status(404)
    .json({error: `joke with id ${id} does not exist. No jokes were deleted`})
  }
})
//8. DELETE All jokes
app.delete('/all',(req,res)=>{
  ...
  if(pass === masterKey){
    ...
    res.send("Invalid authorisation");
    res.status(404);
  }
})
app.listen(port, () => {
